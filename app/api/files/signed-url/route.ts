...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    await logAudit({ event: 'signed_url_requested', entity: 'storage', details: { path, expiresIn } })
    ...
    if (error) {
      ...
      await logAudit({ event: 'signed_url_failed', entity: 'storage', level: 'error', details: { error: error.message } })
      throw new HttpError(500, `Signed URL failed: ${error.message}`)
    }

    await logAudit({ event: 'signed_url_success', entity: 'storage', details: { path } })
    ...
    if (!(err instanceof HttpError)) {
      await logAudit({ event: 'signed_url_failed', entity: 'storage', level: 'error', details: { step: 'unhandled', error: String(err?.message || err) } })
    }
